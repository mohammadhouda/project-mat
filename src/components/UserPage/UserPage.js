...
const UserPage = () => {
  ...
  const handleProjectChange = async (event) => {
    ...
    try {
      const response = await fetch(
        `/mat/api/1.0/private/projects/${encodeURIComponent(project)}/tasks`,
        {
          method: "GET",
          headers: {
      ...
      setError(
        "Failed to fetch tasks for the selected project. " + error.message
      );
    }
  };
  ...
  const handleAddClick = () => {
    if (selectedDate && selectedProject && selectedTask) {
      const taskId = availableTasks.find(
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setError(
          `Failed to submit data. Status: ${response.status}, Error: ${errorText}`
        );
        return;
      }
      ...
      <div className="container">
        ...
        <div className="button-container">
          <button
            ...
            disabled={!selectedDate || !selectedProject || !selectedTask}
          >
            Add
          </button>
        </div>
